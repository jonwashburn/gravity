...
\usepackage[numbers,sort&compress]{natbib} % For bibliography (numeric)
\usepackage{listings}
\usepackage{verbatim}
...
We implement a pure, global-only solver using the production scripts in \texttt{jonwashburn/gravity}: \texttt{active/scripts/ledger\_final\_combined.py} and \texttt{active/scripts/reproduce\_048\_fit.py}. Default configuration uses the acceleration kernel $w_g$, analytic $n(r)$, global $\xi$, $\zeta(r)$ with $h_z/R_d=0.25$, and a single global stellar disk M/L of 1.0. No per-galaxy adjustments are permitted.
...
To demonstrate the concrete existence and reproducibility of our implementation, the code is available at \href{https://github.com/jonwashburn/gravity}{https://github.com/jonwashburn/gravity}. A top-level \texttt{active/} directory contains scripts, data link, and results. The workflow can be run with:

\begin{verbatim}
cd production
python3 -m venv env/.venv && source env/.venv/bin/activate
python -m pip install -r env/requirements.txt
cd scripts
python build_sparc_master_table.py
python ledger_final_combined.py --mode=pure
...
\begin{table}[h]
\centering
\caption{Key repository files used in this analysis (current repo layout).}
\label{tab:files}
\begin{tabular}{l l}
\toprule
Path & Purpose \\
...
active/scripts/ledger\_final\_combined.py & Core ILG pipeline (global-only) \\
active/scripts/reproduce\_048\_fit.py & Reproduce key median result \\
active/results/* & Outputs: logs, pkl, example figures \\
active/paper/* & Paper sources and compiled PDFs \\
\bottomrule
\end{tabular}
\end{table}
...
Example rotation curves (DDO154, NGC3198, Fornax) demonstrate agreement across dwarf and spiral regimes; per-galaxy statistics are included in the CSV artifacts noted above.
...
\bibitem{li2018nature} Li, Q., et al. 2018, \textit{Nature}, 560, 582. Measurements of the gravitational constant using two independent methods.
